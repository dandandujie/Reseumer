...
这个仓库已经不再是原始的 JadeAI 产品，而是收敛为一个更聚焦的简历工作流：保留一个稳定可维护的简历模板，重点服务 macOS / Windows 桌面端使用场景。
...
应用启动时**不要求**预先在服务端写死 AI Key。AI 密钥、Base URL、模型都在应用设置页里配置，并在调用时通过请求头传递。
...
- 历史 README 中的截图和功能描述已经不再适用于这个仓库。
